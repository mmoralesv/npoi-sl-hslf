...
namespace NPOI.HSLF.UserModel
{
     ...
     */
    public class HSLFTableCell : HSLFTextBox implements TableCell<HSLFShape,HSLFTextParagraph> {
   ...
   protected HSLFTableCell(EscherContainerRecord escherRecord, HSLFTable parent){
    ...
    private void anchorBorder(BorderEdge edge, HSLFLine line) {
        if (line == null) {
            return;
        }
    ...
    public StrokeStyle getBorderStyle(final BorderEdge edge) {
        Double width = getBorderWidth(edge);
