...
namespace NPOI.HSLF.UserModel
{
    ...
    * @param data the picture data
    ...
    @Override
    public OutputStream updateObjectData(final Application application, ObjectMetaData metaData) {
        ObjectMetaData md = (application != null) ? application.getMetaData() : metaData;
        if (md == null) {
        ...
        try (InputStream bis = FileMagic.prepareToCheckMagic(baos.toInputStream())) {
            ...
            try (POIFSFileSystem poifs = (fm == FileMagic.OLE2) ? new POIFSFileSystem(bis) : new POIFSFileSystem()) {
                ...
                if (oid == 0) {
                    ...
                    HSLFObjectData od = getObjectData();
                    if (od != null) {
