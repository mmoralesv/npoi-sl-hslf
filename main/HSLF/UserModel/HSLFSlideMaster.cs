...
namespace NPOI.HSLF.UserModel
{
     ...
     */
    public class HSLFSlideMaster : HSLFMasterSheet {
    ...
    private TextPropCollection getPropHelper(final int txtype, int level, String name, boolean isCharacter) {
        ...
        for (int i=minLevel; i >= 0; i--) {
            ...
            TextProp tp = col.findByName(name);
            if (tp != null) {
                return col;
            }
        }
