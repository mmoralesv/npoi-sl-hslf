...
namespace NPOI.HSLF.UserModel
{
     ...
     */
    public class HSLFSlideShowImpl : POIDocument, ICloseable
    {
        //private static Logger LOG = LogManager.getLogger(HSLFSlideShowImpl);

        public static int UNSET_OFFSET = -1;
        ...
        private static int DEFAULT_MAX_RECORD_LENGTH = 200_000_000;
        private static int MAX_DOCUMENT_SIZE = 100_000_000;
        ...
        // Low level contents
        private Record.Record[] _records;
        ...
        public HSLFSlideShowImpl(String fileName)
        ...
        public HSLFSlideShowImpl(InputStream inputStream)
                    ...
                    : base(handleDualStorage(dir))
        {
            ...
            catch (Exception ex) when (ex is RuntimeException || ex is IOException)
            {
                // clean up the filesystem when we cannot read it here to avoid
                // leaking file handles
                dir.FileSystem.Close();

                throw ex;
            }
        }

        private static DirectoryNode handleDualStorage(DirectoryNode dir)
        {
        ...
        private void readPowerPointStream()
        {
            ...
            int len = docProps.Size;
            using (InputStream _is = dir.CreateDocumentInputStream(docProps))
            {
                _docstream = IOUtils.ToByteArray(_is, len);//, MAX_DOCUMENT_SIZE);
            }
        }
        ...
        private Record.Record[] read(byte[] docstream, int usrOffset)
        {
            //sort found records by offset.
            //(it is not necessary but SlideShow.findMostRecentCoreRecords() expects them sorted)
            Dictionary<int, Record.Record> records = new Dictionary<int, Record.Record>();// offset -> record
            Dictionary<int, int> persistIds = new Dictionary<int, int>(); // offset -> persistId
            ...
            foreach (var entry in records)
            {
                ...
                Record.Record record = entry.Value;
                int persistId = persistIds[offset];
                if (record == null)
                {
                    ...
                    record = Record.Record.BuildRecordAtOffset(docstream, offset);
                    records[entry.Key] = record;
                }

                if (record is PersistRecord)
                {
        ...
        private void initRecordOffsets(byte[] docstream, int usrOffset, Dictionary<int, Record.Record> recordMap, Dictionary<int, int> offset2id)
        {
            while (usrOffset != 0)
            {
                Record.Record builtRecord = Record.Record.BuildRecordAtOffset(docstream, usrOffset);
                if (!(builtRecord is UserEditAtom))
                {
                ...
                recordMap.Add(usrOffset, usr);

                int psrOffset = usr.getPersistPointersOffset();
                Record.Record record = Record.Record.BuildRecordAtOffset(docstream, psrOffset);
                if (record == null)
                {
                ...
                recordMap.Add(psrOffset, ptr);

                foreach (var entry in ptr.getSlideLocationsLookup())
                {
                    int offset = entry.Value;
                    int id = entry.Key;
                    recordMap.Add(offset, null); // reserve a slot for the record
                    offset2id.Add(offset, id);
                }
                ...
                if (usrOffset > 0 && recordMap.ContainsKey(usrOffset))
                {
                    ...
                    usrOffset = recordMap.Keys.First() - 36;
        ...
        {
            foreach (Record.Record r in _records)
            {
        ...
        // * @throws IOException If there is an unexpected IOException from writing to the File
        // */
        //public void write(File newFile, boolean preserveNodes)
        //{
         ...
         *                     the passed in OutputStream
         */
        //public void Write(OutputStream _out, bool preserveNodes)
        //{
        //    // Get a new FileSystem to write into

        //    using (POIFSFileSystem outFS = new POIFSFileSystem()) {
        //        // Write into the new FileSystem
        //        Write(outFS, preserveNodes);

        //        // Send the POIFSFileSystem object out to the underlying stream
        //        outFS.WriteFilesystem(_out);
        //    }
        //}

        //    private void Write(POIFSFileSystem outFS, bool copyAllOtherNodes)
        //{
        ...
        //using (HSLFSlideShowEncrypted encryptedSS = new HSLFSlideShowEncrypted(getDocumentEncryptionAtom())) {
        //    _records = encryptedSS.updateEncryptionRecord(_records);

        //    // Write out the Property Streams
        //    writeProperties(outFS, writtenEntries);

        //    using (UnsynchronizedByteArrayOutputStream baos = new UnsynchronizedByteArrayOutputStream()) {
        ...
        public int appendRootLevelRecord(Record.Record newRecord)
        {
            int addedAt = -1;
            Record.Record[] r = new Record.Record[_records.Length + 1];
            bool added = false;
        ...
        public HSLFObjectData[] GetEmbeddedObjects()
        {
            if (_objects == null)
            {
                List<HSLFObjectData> objects = new List<HSLFObjectData>();
                foreach (Record.Record r in _records)
                {
                    if (r is ExOleObjStg)
                    {
        ...
        private EscherContainerRecord getBlipStore()
        {
            ...
            foreach (Record.Record record in _records)
            {
                if (record == null)
                {
            ...
            blipStore = (EscherContainerRecord)HSLFShape.GetEscherChild(dggContainer, EscherContainerRecord.BSTORE_CONTAINER);
            if (blipStore == null)
            {
        ...
        public override void Close()
        {
            ...
            if (Directory.Parent == null ||
                PP97_DOCUMENT.Equals(Directory.Name))
            {
                POIFSFileSystem fs = (POIFSFileSystem)Directory.FileSystem;
                if (fs != null)
                {
                    fs.Close();
                }
        ...
        public void WritePropertiesImpl()
        {
            base.WriteProperties();
        }

        public PropertySet GetPropertySetImpl(String setName)
        {
            return base.GetPropertySet(setName);
        }

        public PropertySet GetPropertySetImpl(String setName, EncryptionInfo encryptionInfo)
        {
            return base.GetPropertySet(setName, encryptionInfo);
        }

        public void WritePropertiesImpl(POIFSFileSystem outFS, List<String> writtenEntries)
        {
            base.WriteProperties(outFS, writtenEntries);
        }

        public void ValidateInPlaceWritePossibleImpl()
        {
            base.ValidateInPlaceWritePossible();
        }
        ...
        //bool InitDirectoryImpl()
        //{
        //    return base.InitDirectory();
        //}

        //void ReplaceDirectoryImpl(DirectoryNode newDirectory)
        //{
        //    base.ReplaceDirectory(newDirectory);
        //}
        ...
        //private static class CountingOS : OutputStream
        //{
        //    int count;
        ...
        //    private EscherContainerRecord recordContainer;
        //        private PictureData.PictureType type;
        //    private int offset;
